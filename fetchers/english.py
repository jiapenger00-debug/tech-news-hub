...
class EnglishNewsFetcher(BaseNewsFetcher):
    """英文科技新闻抓取器 - 严格过滤"""
    ...
    def fetch(self):
        """抓取英文新闻"""
        news_list = []
        sources = self.config["sources"]["english"]
    ...
    def _fetch_web(self, source, max_articles):
        ...
        # 根据 The Verge 的结构定制
        if "theverge" in source["url"]:
            articles = soup.find_all("div", class_="duet--content-cards--content-card", limit=max_articles)
                ...
                try:
                    ...
                    if title_elem and link_elem:
                        ...
                        if self.is_relevant(title, summary):
                            news_list.append({
                                "title": title,
                                "url": "https://www.theverge.com" + link_elem.get("href", ""),
                                "summary": summary,
                                "published": "",
                                "source": source["name"],
                                "language": "en",
